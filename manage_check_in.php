        ...
        <!-- Breadcrumbs -->
            <nav aria-label="breadcrumb" role="navigation">
              <ol class="breadcrumb" style="padding: 0; background: none; margin-top: -20px;">
                ...
                <li class="breadcrumb-item active float-right" aria-current="page"><i class="fa fa-file"></i> Check In</li>
              </ol>
            </nav>
            <div class="card">
                        <div class="card-header bg-red-dark text-white">
                            <strong class="h3"> Active Guests </strong>
                            <div class="d-inline float-right">
                                <a href="new_check_in.php" class="btn btn-success"> <i class="fa fa-plus"></i> Check In</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive dataTable">
                                <table class= "table table-striped table-bordered table-hover table-sm text-center" id="reservation_table" cellspacing="0" width="100%" >
                                    ...
                                    <tbody>
                                        <?php
                                            $sql = mysqli_query($conn, "SELECT * FROM `tbl_reservations` rs LEFT JOIN `tbl_rooms` r ON rs.`room_number` = r.`room_id` WHERE `status` = 'active'  ORDER BY rs.`trans_date` DESC");
                                            ...
                                            if ($_SESSION['user_type'] == "administrator") {
                                               echo'<td><a href="reservation_details.php?id='.$row['id'].'" class="btn btn-xs btn-info"><i class="fa fa-edit"></i> Edit</a> <a href="app/delete_check_in_details.php?id='.$row['id'].'" class="btn btn-xs btn-danger">Delete</a> </td>';
                                            }else{
                                                 echo'<td><a href="reservation_details.php?id='.$row['id'].'" class="btn btn-xs btn-info"><i class="fa fa-edit"></i> Edit</a></td>';
                                            }
